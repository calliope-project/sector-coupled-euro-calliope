#!/bin/sh
#BSUB -J ehighways_resolution_test[1-5]
#BSUB -n 6
...
calliope run --scenario "industry_fuel_isolated,transport,heat,config_overrides,gas_storage,freeze-hydro-capacities,${array[${LSB_JOBINDEX}-1]}" --save_netcdf "${DIR}/outputs/resolution_test/${array[${LSB_JOBINDEX}-1]}.nc" "${DIR}/build/model/ehighways/model.yaml"
