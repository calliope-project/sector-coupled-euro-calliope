...
mkdir -p "${DIR}/build/ehighways/spores_2h" "${DIR}/build/logs/spores_2h" "${DIR}/build/ehighways/spores_2h/${LSB_JOBINDEX}-${array[${LSB_JOBINDEX}-1]}"

calliope run --scenario "transport,heat,config_overrides,gas_storage,freeze-hydro-capacities,link_cap_dynamic,res_2h,add-biofuel,${array[${LSB_JOBINDEX}-1]}" --save_netcdf "${DIR}/build/ehighways/spores_2h/${LSB_JOBINDEX}-${array[${LSB_JOBINDEX}-1]}.nc" "${DIR}/build/model/ehighways/model.yaml"
