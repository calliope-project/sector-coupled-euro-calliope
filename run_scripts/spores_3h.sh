...
#BSUB -W 1440
#BSUB -r
#BSUB -o /cluster/work/cpesm/brynmorp/euro-spores/logs/spores_3h/log_%I.log
...
mkdir -p "${DIR}/outputs/spores_3h" "${DIR}/logs/spores_3h" "${DIR}/outputs/spores_3h/${LSB_JOBINDEX}-${array[${LSB_JOBINDEX}-1]}"

calliope run --scenario "transport,heat,config_overrides,gas_storage,freeze-hydro-capacities,link_cap_dynamic,res_3h,${array[${LSB_JOBINDEX}-1]}" --save_netcdf "${DIR}/outputs/spores_3h/${LSB_JOBINDEX}-${array[${LSB_JOBINDEX}-1]}.nc" "${DIR}/build/model/eurospores/model.yaml"
