#!/bin/sh
#BSUB -J ehighways_link_cap_shared[1-11]
#BSUB -n 6
...
calliope run --scenario "industry_fuel_${fuel},transport,heat,config_overrides,res_2h,gas_storage,freeze-hydro-capacities,${array[${LSB_JOBINDEX}-1]}" --save_netcdf "${DIR}/outputs/link_cap_test_${fuel}/${array[${LSB_JOBINDEX}-1]}.nc" "${DIR}/build/model/ehighways/model.yaml"
